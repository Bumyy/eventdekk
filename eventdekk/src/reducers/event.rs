...
pub fn create_event(
    ...
    info!(
        "Event '{}' (ID: {}) created by user {:?} for group {}",
        inserted_event.name, new_event_id, ctx.sender, creator_group_id
    );
    ...
    for sub_data in sub_events_data {
        if sub_data.name.trim().is_empty() {
            return Err(format!(
                "SubEvent name '{}' cannot be empty.",
                sub_data.name
            ));
        }
        if sub_data.scheduled_start_time >= sub_data.scheduled_end_time {
            return Err(format!(
                "SubEvent '{}' start time must be before end time.",
                sub_data.name
            ));
        }
        if sub_data.scheduled_start_time < inserted_event.start_time
            || sub_data.scheduled_end_time > inserted_event.end_time
        {
            warn!(
                "SubEvent '{}' times fall outside parent event times.",
                sub_data.name
            );
        }
        ...
        match sub_data.sub_event_type {
            SubEventType::FlyIn | SubEventType::FlyOut => {
                let hub = sub_data.hub_icao.ok_or(format!(
                    "hub_icao is required for FlyIn/FlyOut SubEvent '{}'.",
                    sub_data.name
                ))?;
                if hub.len() != 4 {
                    return Err(format!(
                        "hub_icao for SubEvent '{}' must be 4 letters.",
                        sub_data.name
                    ));
                }
            ...
            SubEventType::GroupFlight => {
                let dep = sub_data.group_flight_departure_icao.ok_or(format!(
                    "group_flight_departure_icao is required for GroupFlight SubEvent '{}'.",
                    sub_data.name
                ))?;
                let arr = sub_data.group_flight_arrival_icao.ok_or(format!(
                    "group_flight_arrival_icao is required for GroupFlight SubEvent '{}'.",
                    sub_data.name
                ))?;
                if dep.len() != 4 {
                    return Err(format!(
                        "group_flight_departure_icao for SubEvent '{}' must be 4 letters.",
                        sub_data.name
                    ));
                }
                if arr.len() != 4 {
                    return Err(format!(
                        "group_flight_arrival_icao for SubEvent '{}' must be 4 letters.",
                        sub_data.name
                    ));
                }
...
pub fn update_event(
    ...
    if status == EventStatus::Published && event.status != EventStatus::Published {
        check_permission(ctx, event.creator_group_id, PermissionLevel::CEO)?;
    }
    ...
    if (status != EventStatus::Published || is_internal)
        && ctx.db.discovery_event().event_id().find(event_id).is_some()
    {
        ctx.db.discovery_event().event_id().delete(event_id);
        info!(
            "Event {} removed from discovery due to status change.",
            event_id
        );
    }
    Ok(())
...
pub fn invite_group_to_event(
    ...
    info!(
        "Group {} invited to event {} by user {:?}",
        invited_group_id, event_id, ctx.sender
    );
    Ok(())
}
...
pub fn respond_to_event_invitation(
    ...
    let participation = ctx
        ...
        .ok_or_else(|| {
            format!(
                "No pending invitation found for group {} to event {}.",
                group_id, event_id
            )
        })?;
    ...
    ctx.db
        .event_participant()
        .participation_id()
        .update(updated_participation);

    info!(
        "Group {} responded {:?} to event {} invitation",
        group_id, response, event_id
    );
    Ok(())
}
...
pub fn add_sub_event(
    ...
    hub_icao: Option<String>,
    group_flight_departure_icao: Option<String>,
    group_flight_arrival_icao: Option<String>,
    group_flight_route: Option<String>,
    notes: Option<String>,
) -> Result<(), String> {
    is_event_host_staff_or_ceo(ctx, event_id)?;
    ...
    if name.trim().is_empty() {
        return Err("SubEvent name cannot be empty.".to_string());
    }
    if scheduled_start_time >= scheduled_end_time {
        return Err("SubEvent start time must be before end time.".to_string());
    }
    ...
    match sub_event_type {
        SubEventType::FlyIn | SubEventType::FlyOut => {
            let hub =
                hub_icao.ok_or("hub_icao is required for FlyIn/FlyOut SubEvents.".to_string())?;
            if hub.len() != 4 {
                return Err("hub_icao must be 4 letters.".to_string());
            }
            validated_hub_icao = Some(hub);
            validated_gf_dep_icao = None;
            validated_gf_arr_icao = None;
        ...
        SubEventType::GroupFlight => {
            let dep = group_flight_departure_icao
                .ok_or("group_flight_departure_icao is required.".to_string())?;
            let arr = group_flight_arrival_icao
                .ok_or("group_flight_arrival_icao is required.".to_string())?;
            if dep.len() != 4 || arr.len() != 4 {
    ...
    info!(
        "SubEvent '{}' added to Event {}",
        inserted_sub_event.name, event_id
    );
    Ok(())
}
...
pub fn delete_sub_event(ctx: &ReducerContext, sub_event_id: u64) -> Result<(), String> {
    ...
    {
        return Err(format!(
            "Cannot delete sub-event {} as it has flight signups.",
            sub_event_id
        ));
    }
    ...
    info!(
        "SubEvent ID {} deleted from Event {}",
        sub_event_id, sub_event.event_id
    );
    Ok(())
}
