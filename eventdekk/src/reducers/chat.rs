...
pub fn add_live_chat_message(
    ...
    if message.len() > 500 {
        return Err("Message exceeds maximum length.".to_string());
    }
    if ctx.db.event().event_id().find(event_id).is_none() {
        return Err(format!("Event {} not found.", event_id));
    }
    if ctx.db.group().group_id().find(group_id).is_none() {
