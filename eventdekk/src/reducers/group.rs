...
use crate::tables::{user, group, Group, group_membership, GroupMembership};
...
#[reducer]
pub fn update_group(
    ctx: &ReducerContext, group_id: u64, name: String, tag: String,
    description: String, website_url: Option<String>, logo_url: Option<String>,
    color: Option<String>,
) -> Result<(), String> {
    check_permission(ctx, group_id, PermissionLevel::Staff)?;
    let mut group = find_group_or_err(ctx, group_id)?;
    
    if name.trim().is_empty() { return Err("Group name cannot be empty.".to_string()); }
...
pub fn add_group_member(
    ctx: &ReducerContext, group_id: u64, user_identity: Identity,
