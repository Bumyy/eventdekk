...
use crate::tables::{
    discovery_event, discovery_rotation_schedule, event, event_participant, DiscoveryEvent,
    DiscoveryRotationSchedule, Event,
};
...
fn schedule_discovery_rotation(ctx: &ReducerContext) {
    ...
    if ctx.db.discovery_rotation_schedule().iter().count() == 0 {
        ctx.db
            .discovery_rotation_schedule()
            .insert(DiscoveryRotationSchedule {
                scheduled_id: 0,
...
fn populate_initial_discovery(ctx: &ReducerContext) {
    ...
    let mut candidates: Vec<Event> = ctx
        ...
        .iter()
        .filter(|e| {
            e.status == EventStatus::Published && !e.is_internal && e.start_time > min_start_time
        })
        .collect();
...
pub fn rotate_discovery_event(
    ...
    debug!(
        "Running discovery rotation (Schedule ID: {})...",
        schedule_info.scheduled_id
    );
    let now = ctx.timestamp; // now is spacetimedb::Timestamp

    // --- 1. Cleanup ---
    let mut needs_re_prioritize = false;
    let discovery_item_ids: Vec<u64> = ctx
        .db
        .discovery_event()
        .iter()
    ...
    let mut current_discovery_events: Vec<DiscoveryEvent> =
        ctx.db.discovery_event().iter().collect();
    ...
    if !current_discovery_events.is_empty() {
        ...
        let events_to_update_ids: Vec<u64> = current_discovery_events
            .iter()
            .map(|e| e.event_id)
            .collect();

        debug!(
            "Re-prioritizing {} remaining discovery events...",
            events_to_update_ids.len()
        );
        for event_id in events_to_update_ids {
            ...
            if let Some(mut event_entry) = ctx.db.discovery_event().event_id().find(event_id) {
                if event_entry.display_priority != new_priority {
                    debug!(
                        "Updating priority for Event ID {}: {} -> {}",
                        event_entry.event_id, event_entry.display_priority, new_priority
                    );
                ...
                warn!(
                    "Event ID {} disappeared during re-prioritization?",
                    event_id
                );
            }
        }
    ...
    if needed > 0 {
        debug!(
            "Discovery list has {} items, need to fill {} slots.",
            current_count, needed
        );

        // Find eligible events that are not already in the discovery list
        let current_discovery_ids: HashSet<u64> = current_discovery_events
            .iter()
            .map(|e| e.event_id)
            .collect();
        let min_start_time = now + TimeDuration::from_micros(ELIGIBILITY_LOOKAHEAD_MICROS);

        let mut potential_candidates: Vec<Event> = ctx
            .db
            .event()
            .iter()
            .filter(
                |e| {
                    e.status == EventStatus::Published
                        ...
                        && !current_discovery_ids.contains(&e.event_id)
        ...
        for candidate in potential_candidates.iter().take(needed) {
            debug!(
                "Adding Event ID {} with priority {} to fill discovery.",
                candidate.event_id, current_priority_to_assign
            );
        ...
        debug!(
            "Discovery list is full ({} items). No filling needed.",
            current_count
        );
    }
    ...
    debug!(
        "Swap-out candidates (EventID, Weight): {:?}",
        swap_out_candidates
    );

    let swap_out_index_opt =
        select_weighted_random_index(&swap_out_candidates, schedule_info.scheduled_id);
    ...
    debug!(
        "Selected Event ID {} (Priority {}) to potentially swap out.",
        swap_out_event_id, swap_out_priority
    );
    ...
    let min_start_time_for_candidate =
        now + TimeDuration::from_micros(ELIGIBILITY_LOOKAHEAD_MICROS);

    let all_eligible_events: Vec<Event> = ctx
        .db
        .event()
        .iter()
        .filter(|e| {
            e.status == EventStatus::Published
                && !e.is_internal
                && e.start_time > min_start_time_for_candidate
        })
        .collect();
    ...
    let replacement_pool: Vec<&Event> = all_eligible_events
        .iter()
        .filter(|e| !current_discovery_ids.contains(&e.event_id) || e.event_id == swap_out_event_id)
        .collect();

    debug!(
        "Found {} potential replacement candidates.",
        replacement_pool.len()
    );
    ...
    let replacement_candidates_with_weights: Vec<(u64, u64)> = replacement_pool
        ...
        .map(|&event| {
            // event is type Event
            let participant_count = group_participant_counts
                .get(&event.event_id)
                .cloned()
                .unwrap_or(0);
            ...
            let age_seconds = if age_micros < 0 {
                warn!(
                    "Calculated negative age_micros {} for event {}. Using 0.",
                    age_micros, event.event_id
                );
                0u64
            } else {
    ...
    debug!(
        "Replacement candidates with weights (EventID, Weight): {:?}",
        replacement_candidates_with_weights
    );

    // Corrected: Use to_micros_since_unix_epoch()
    let timestamp_micros = now.to_micros_since_unix_epoch(); // Returns i64
    let timestamp_seed_part = if timestamp_micros < 0 {
        0u64
    } else {
    ...
    let replacement_index_opt =
        select_weighted_random_index(&replacement_candidates_with_weights, replacement_seed);

    let replacement_event_id = match replacement_index_opt {
        Some(index) => replacement_pool
            .get(index)
            .map(|e| e.event_id)
            .ok_or_else(|| {
                format!(
                    "Internal error: Invalid index {} selected for replacement.",
                    index
                )
            })?,
        None => {
    ...
    debug!(
        "Selected Event ID {} as replacement candidate.",
    ...
    if current_discovery_ids.contains(&replacement_event_id)
        && replacement_event_id != swap_out_event_id
    {
    ...
    ctx.db
        .discovery_event()
        .event_id()
        .delete(swap_out_event_id);
    debug!(
        "Removed swap-out Event ID {} from discovery.",
        swap_out_event_id
    );
    ...
    ctx.db.discovery_event().insert(new_entry);
    info!(
        "Swapped Discovery Event ID {} (at Priority {}) with new Event ID {}",
        swap_out_event_id, swap_out_priority, replacement_event_id
    );
